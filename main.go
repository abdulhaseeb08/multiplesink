...
func buildPipeline() (*gst.Pipeline, error) {
	...
	decodebin.Connect("pad-added", func(self *gst.Element, srcPad *gst.Pad) {
		...
		if audioStream {
			...
			elements, err := gst.NewElementMany("queue", "audioconvert", "audioresample", "tee")
			if err != nil {
			...
			elementsMu, err := gst.NewElementMany("queue", "queue", "autoaudiosink", "autoaudiosink")
			if err != nil {
				msg := gst.NewErrorMessage(self, gst.NewGError(2, err), "Could not create elements for audio pipeline", nil)
				pipeline.GetPipelineBus().Post(msg)
				return
			...
			pipeline.AddMany(elementsMu...)

			//Link the queues and audiosinks
			elementsMu[0].Link(elementsMu[2])
			elementsMu[1].Link(elementsMu[3])

			//Request source pads from the tee element and send the output to the relevant sink
			teepadOne := elements[3].GetRequestPad("src_%u")
			queuepadOne := elementsMu[0].GetStaticPad("sink")
			teepadOne.Link(queuepadOne)
			teepadTwo := elements[3].GetRequestPad("src_%u")
			queuepadTwo := elementsMu[1].GetStaticPad("sink")
			teepadTwo.Link(queuepadTwo)

			for _, e := range elementsMu {
				e.SyncStateWithParent()
			}
		...
		if videoStream {
			//we create the following pipeline if audioStream is found
			elements, err := gst.NewElementMany("queue", "videoconvert", "videoscale", "tee")
			if err != nil {
			...
			elementsMu, err := gst.NewElementMany("queue", "queue", "autovideosink", "autovideosink")
			if err != nil {
				msg := gst.NewErrorMessage(self, gst.NewGError(2, err), "Could not create elements for audio pipeline", nil)
				pipeline.GetPipelineBus().Post(msg)
				return
			...
			//Request source pads from tee and and connect them to the relevant sinks
			teepadOne := elements[3].GetRequestPad("src_%u")
			queuepadOne := elementsMu[0].GetStaticPad("sink")
			teepadOne.Link(queuepadOne)
			teepadTwo := elements[3].GetRequestPad("src_%u")
